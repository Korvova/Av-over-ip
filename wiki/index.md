...
**Объект, сентябрь 2026** (8 устройств, 4 монитора): платформа доведена на реальной
сети — автопоиск «под ключ», многоадресный режим включается кнопкой, коммутация идёт
по IP энкодера (по номеру канала декодеры хост не находили), адреса устройств
отслеживаются по MAC (в автоматическом режиме они меняются), статусы и активность
живые, видеостена включается и выключается кнопкой и занимает свои декодеры в матрице,
превью потоков в таблице устройств, сессия входа переживает перезагрузку страницы.
Уроки — в колонке «Урок» страниц требований и в разделе «Протокол HDN-EA900».

В работе: резервирование пары серверов, обновление из файла для объектов без
интернета, поддержка второго производителя (BeingHD, ждём ответа завода).
...
❌ Исключено (с датой и причиной; строки не удаляются).
Ведём по стандарту [Культура ведения проектов](/homepage/servisy/api/kultura-vedenija-proektov):
новая правка — новая строка, картинки внутри строк требований, хеш коммита в «Уроке».
