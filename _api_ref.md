| 履历页 | 履历页 | 履历页 | 履历页 | 履历页 | 履历页 |
| 序号 | 更改内容摘要描述 | 更改内容摘要描述 | 版本 | 修订人 | 修订日期 |
| 1 | 新版发行 | 新版发行 | 1.0 | 陈万培 | 2024-03-19 |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
...
|  |  |  |  |  |  |
|  |  |  |  |  |  |
| 批 准：    日 期： | 批 准：    日 期： | 审 核：    日 期： | 制 作：    日 期： | 制 作：    日 期： | 制 作：    日 期： |
...
Setting and Obtaining the HDMI OUT HDCP function	31

Setting HDMI OUT HDCP Rule	31

Getting HDMI OUT HDCP Rule	34
...
| SET API | SET API |
| astparam s en_ssh xxx | astparam s en_ssh xxx |
| describe | describe |
| Enable or disable the SSH service. | Enable or disable the SSH service. |
| parameter | describe |
...
| SET API | SET API |
| astparam s en_telnet xxx | astparam s en_telnet xxx |
| describe | describe |
| Enable or disable the Telnet service. | Enable or disable the Telnet service. |
| parameter | describe |
...
| SET API | SET API |
| astparam s irmode xxx | astparam s irmode xxx |
| describe | describe |
| Select IR wire(e.g. 5v or 12v)  The 5v wire can be set or obtained through ir5vtype, and the default is the factory configuration. " unstandard" means HDCVT 5v IR wire. astparam s ir5vtype standard astparam s ir5vtype unstandard | Select IR wire(e.g. 5v or 12v)  The 5v wire can be set or obtained through ir5vtype, and the default is the factory configuration. " unstandard" means HDCVT 5v IR wire. astparam s ir5vtype standard astparam s ir5vtype unstandard |
| parameter | describe |
...
| 0 | success |
| example | example |
| Setup Time: This means that you need to restart the device after modifying the instructions. 1.Use 5v IR wire astparam s irmode 5v astparam save reboot -f  1. Use 12v IR wire astparam s irmode 12v astparam save reboot -f  Runtime: This means that changes to the directive take effect immediately. 1. Use 5v IR unstandard wire e e_irmode::5v::unstandard   2. Use 5v IR standard wire e e_irmode::5v::standard  3. Use 12v IR wire e e_irmode::12v | Setup Time: This means that you need to restart the device after modifying the instructions. 1.Use 5v IR wire astparam s irmode 5v astparam save reboot -f  1. Use 12v IR wire astparam s irmode 12v astparam save reboot -f  Runtime: This means that changes to the directive take effect immediately. 1. Use 5v IR unstandard wire e e_irmode::5v::unstandard   2. Use 5v IR standard wire e e_irmode::5v::standard  3. Use 12v IR wire e e_irmode::12v |

## Getting IR wire mode

| GET API | GET API |
| astparam g irmode | astparam g irmode |
| describe | describe |
| Obtain the IR wire mode. | Obtain the IR wire mode. |
| return value | describe |
| not defined | Default is 12v |
| 12v/5v | 12v: Use 12v IR wire. 5v: Use 5v IR wire. |
| example | example |
| astparam g irmode y  The 5v wire can be set or obtained through ir5vtype, and the default is the factory configuration. astparam g ir5vtype standard  astparam g ir5vtype unstandard | astparam g irmode y  The 5v wire can be set or obtained through ir5vtype, and the default is the factory configuration. astparam g ir5vtype standard  astparam g ir5vtype unstandard |
...
| SET API | SET API |
| astparam s en_autocap xxx | astparam s en_autocap xxx |
| describe | describe |
| Enable capture mode. (ast1520) It will be replace by MJPEG stream on ast1530+ast1535 chip platform. Use /www/cap.bmp to get picture. It will be taking effect on reboot system. | Enable capture mode. (ast1520) It will be replace by MJPEG stream on ast1530+ast1535 chip platform. Use /www/cap.bmp to get picture. It will be taking effect on reboot system. |
| parameter | describe |
...
| 0 | success |
| example | example |
| Setup Time: This means that you need to restart the device after modifying the instructions. 1.Enable capture mode astparam s en_autocap y astparam save reboot -f  1. Disable capture mode astparam s en_autocap n astparam save reboot -f | Setup Time: This means that you need to restart the device after modifying the instructions. 1.Enable capture mode astparam s en_autocap y astparam save reboot -f  1. Disable capture mode astparam s en_autocap n astparam save reboot -f |

## Getting Capture function

| GET API | GET API |
| astparam g en_autocap | astparam g en_autocap |
| describe | describe |
| Obtain capture mode. (ast1520) It will be replace by mjeg stream on ast1530+ast1535 chip platform. Use /www/cap.bmp to get picture. It will be take effect on reboot system. | Obtain capture mode. (ast1520) It will be replace by mjeg stream on ast1530+ast1535 chip platform. Use /www/cap.bmp to get picture. It will be take effect on reboot system. |
| return value | describe |
| not defined | Default is y. |
| y/n | y=Enable capture function.  n= Disable capture function. |
| example | example |
| astparam g en_autocap y | astparam g en_autocap y |
...
| SET API | SET API |
| astparam s xxx yyy  xxx: "xxx" is the Ast parameter in the following table. yyy: "yyy" is the Value in the following table. | astparam s xxx yyy  xxx: "xxx" is the Ast parameter in the following table. yyy: "yyy" is the Value in the following table. |
| describe | describe |
| See the above table for specific parameters. | See the above table for specific parameters. |
| return value | describe |
| 0 | success |
| example | example |
| Setup Time: This means that you need to restart the device after modifying the instructions. 1.Enable Dante mode astparam s dante_trial_mode y astparam s a_addon dante astparam save reboot -f | Setup Time: This means that you need to restart the device after modifying the instructions. 1.Enable Dante mode astparam s dante_trial_mode y astparam s a_addon dante astparam save reboot -f |
...
| SET API | SET API |
| astparam s en_lldp xxx | astparam s en_lldp xxx |
| describe | describe |
| Link Layer Discovery Protocol (LLDP) is a layer 2 neighbor discovery protocol that allows devices to advertise device information to their directly connected peers/neighbors. It is best practice to enable LLDP globally to standardize network topology across all devices if you have a multi-vendor network. | Link Layer Discovery Protocol (LLDP) is a layer 2 neighbor discovery protocol that allows devices to advertise device information to their directly connected peers/neighbors. It is best practice to enable LLDP globally to standardize network topology across all devices if you have a multi-vendor network. |
| parameter | describe |
...
| example | example |
| astparam g en_lldp y | astparam g en_lldp y |
...
| SET API | SET API |
| astparam s en_lighttpd xxx | astparam s en_lighttpd xxx |
| describe | describe |
| Enable/Disable built-in web services. | Enable/Disable built-in web services. |
| parameter | describe |
...
| GET API | GET API |
| astparam g en_lighttpd | astparam g en_lighttpd |
| describe | describe |
| Obtain the web service status. | Obtain the web service status. |
| return value | describe |
| not defined | Default is y. |
| y/n | y=Enable web service function.  n= Disable web service function. |
| example | example |
| astparam g en_lighttpd y | astparam g en_lighttpd y |
...
| SET API | SET API |
| astparam s relay1status xxx astparam s relay2status xxx | astparam s relay1status xxx astparam s relay2status xxx |
| describe | describe |
| Open/Close the relay1 or relay2. | Open/Close the relay1 or relay2. |
| parameter | describe |
...
| SET API | SET API |
| astparam s iolevel xxx astparam s io1mode yyy astparam s io2mode yyy astparam s io1status zzz astparam s io2status zzz | astparam s iolevel xxx astparam s io1mode yyy astparam s io2mode yyy astparam s io1status zzz astparam s io2status zzz |
| describe | describe |
| Setting Digital IO function | Setting Digital IO function |
| parameter | describe |
...
| example | example |
| astparam g io1status y | astparam g io1status y |
...
| SET API | SET API |
| astparam s en_mcudebuglog xxx | astparam s en_mcudebuglog xxx |
| describe | describe |
| Enable/Disable mcu debug status. | Enable/Disable mcu debug status. |
| parameter | describe |
...
| SET API | SET API |
| astparam s led_on xxx astparam s led_timer yyy astparam s panellock zzz | astparam s led_on xxx astparam s led_timer yyy astparam s panellock zzz |
| describe | describe |
| Enable/Disable mcu debug status. | Enable/Disable mcu debug status. |
| parameter | describe |
...
| SET API | SET API |
| e e_fc_mode_select::xxx | e e_fc_mode_select::xxx |
| describe | describe |
| Set up network transmission using fiber or copper. | Set up network transmission using fiber or copper. |
| parameter | describe |
| xxx = fiber | Use Fiber. |
...
| 0 | success |
| example | example |
| Runtime: This means that changes to the directive take effect immediately. 1.Setting fiber mode. e e_fc_mode_select::fiber  2.Setting copper mode. e e_fc_mode_select::copper | Runtime: This means that changes to the directive take effect immediately. 1.Setting fiber mode. e e_fc_mode_select::fiber  2.Setting copper mode. e e_fc_mode_select::copper |

## Getting Fcmode function

| GET API | GET API |
| astparam g fcmode | astparam g fcmode |
| describe | describe |
| To obtain network transmission, use fiber or copper. | To obtain network transmission, use fiber or copper. |
| return value | describe |
| not defined | Default is copper. |
| fiber/copper | fiber=Use fiber.  copper=Use copper. |
| example | example |
| astparam g fcmode copper | astparam g fcmode copper |
...
| SET API | SET API |
| e e_edid_select::xxx | e e_edid_select::xxx |
| describe | describe |
| Taking effect. | Taking effect. |
| parameter | describe |
...
| 0 | success |
| example | example |
| Runtime: This means that changes to the directive take effect immediately. e e_edid_select::1080PPCM20SDR | Runtime: This means that changes to the directive take effect immediately. e e_edid_select::1080PPCM20SDR |

## Getting Edid

| GET API | GET API |
| astparam g edid | astparam g edid |
| describe | describe |
| Obtain the edid | Obtain the edid |
| return value | describe |
...
| example | example |
| astparam g edid 00 | astparam g edid 00 |

## Setting HDMI OUT CEC guest mode code

| SET API | SET API |
| e e_hdmiout_cec_code::xxx | e e_hdmiout_cec_code::xxx |
| describe | describe |
| Taking effect. | Taking effect. |
| parameter | describe |
| xxx | cec code. |
...
| 0 | success |
| example | example |
| Runtime: This means that changes to the directive take effect immediately.  1. Open TV e e_hdmiout_cec_code::40_04  2.Close TV e e_hdmiout_cec_code::40_36 | Runtime: This means that changes to the directive take effect immediately.  1. Open TV e e_hdmiout_cec_code::40_04  2.Close TV e e_hdmiout_cec_code::40_36 |
...
| SET API | SET API |
| astparam s hdmiouthdcp xxx | astparam s hdmiouthdcp xxx |
| describe | describe |
| Setting HDMI out hdcp rules. | Setting HDMI out hdcp rules. |
| parameter | describe |
...
| example | example |
| astparam g hdmiouthdcp hdcp_src | astparam g hdmiouthdcp hdcp_src |
...
| SET API | SET API |
| e e_video_genlock_scaling::xxx | e e_video_genlock_scaling::xxx |
| describe | describe |
| Select the Video Out Resolution | Select the Video Out Resolution |
| parameter | describe |
...
| 0 | success |
| example | example |
| Runtime: This means that changes to the directive take effect immediately. e e_video_genlock_scaling::1080P50 | Runtime: This means that changes to the directive take effect immediately. e e_video_genlock_scaling::1080P50 |

## Getting Video Out Resolution

| GET API | GET API |
| astparam g resolution | astparam g resolution |
| describe | describe |
| Obtain the resolution. | Obtain the resolution. |
| return value | describe |
| xxx | Default is 00. It means video out resolution bypass. 00：bypass 01：1080P50 02：1080P60 03：720P50 04：720P60 05：2160P24 06：2160P30 07：2160P50 08：2160P60 09：1280x1024 10：1360x768 11：1440x900 12：1680x1050 13：1920x1200 |
| example | example |
| astparam g resolution 01 | astparam g resolution 01 |
...
| SET API | SET API |
| astparam s hdcp_always_on aaa astparam s hdcp_always_on_22 bbb astparam s hdcp_always_off ccc astparam s hdcp_always_on_src ddd astparam s hdcp_always_on_snk eee | astparam s hdcp_always_on aaa astparam s hdcp_always_on_22 bbb astparam s hdcp_always_off ccc astparam s hdcp_always_on_src ddd astparam s hdcp_always_on_snk eee |
| describe | describe |
| Setting HDMI out hdcp rules. | Setting HDMI out hdcp rules. |
| parameter | describe |
